...
def create_zip_list(event_folder):
    ...
    """
    Based on a folder with events in xml.zip format,
    it creates a list of paths for each xml.zip file.
...
def zip2catalog(file_path):
    ...
    """
    Uncompress the zip file, convert the xml to a
    obspy.core.event object

    :param file_path: contains the zip file path
    :returns: temp_event
    :rtype: obspy.core.event object
    """
...
def add_modification_time(event_xml_uncompressed):
    ...
    """
    Add an event modification time.

    :param event_xml_uncompressed: event data in XML format
    :returns: modification_time
    :rtype: UTCDateTime object
    """
...
def fdsn_network_magnitude(event):
    ...
    """
    Creates a dictionary with information of an event
    and stores it in the ``network_magnitude`` list.

    :param event: obspy.core.event object
    """
...
def add_event_info2station_magnitude(event):
    ...
    """
    Stores the event information into station magnitud list.
...
def create_station_magnitude_df(event_list):
    ...
    """
    Taking a ``event_list``, it creates a dataframe
    of the magnitude of the station.

    :param event_list: event list
    :returns: station_mag_df
    :rtype: dataframe
    """
    ...
    st_mag_df['station_id'] = st_mag_df['waveform_id'].\
        apply(lambda x: x.network_code + "_" + x.station_code + "_" + x.channel_code)
    ...
    st_mag_df['modification_time'] = st_mag_df['extra'].\
        apply(lambda x: x['modification_time'] if x['modification_time'] else np.nan)

    st_mag_df['event_creation_time'] = st_mag_df['extra'].\
        apply(lambda x: x['event_creation_time'] if x else np.nan)
    ...
    station_mag_df = \
        st_mag_df[['creation_time', 'station_id', 'mag', 'station_magnitude_type',
                   'author', 'event_id', 'modification_time', 'event_creation_time',
                   'amplitude_id']].copy()
    ...
    station_mag_df['modification_time'] = \
        station_mag_df['modification_time'].apply(lambda x: x.isoformat())
    station_mag_df['event_creation_time'] = \
        station_mag_df['event_creation_time'].apply(lambda x: x.isoformat())
...
def create_network_magnitude_df(event_network_mag_list):
    ...
    """
    Taking a ``event_network_mag_list``, it creates a dataframe
    of the magnitude of the network.

    :param event_network_mag_list: event network magnitude list
    :returns: event_network_mag_df
    :rtype: dataframe
    """
    ...
    event_network_mag_df.modification_time = \
        event_network_mag_df.modification_time.apply(lambda x: x.isoformat())
    event_network_mag_df.event_creation_time = \
        event_network_mag_df.event_creation_time.apply(lambda x: x.isoformat())
    # COMO OBTENER VALORES POR TAMANO DE MAGNITUD
    event_network_mag_df['mag_diff'] = event_network_mag_df['mag']\
        - event_network_mag_df['mag'].mean()
...
def create_amplitude_df(event_list):
    ...
    """
    Combine dataframes of the event amplitudes and return
    a combine dataframe.

    :param event_list: event list
    :returns: pd.concat(temp_list)
    :rtype: dataframe
    """
